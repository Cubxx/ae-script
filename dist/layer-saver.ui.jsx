...
(function (that) {
    ...
    var KeyframeData = (function () {
        ...
        KeyframeData.isSpatial = function (p) {
            return !![PropertyValueType.TwoD_SPATIAL, PropertyValueType.ThreeD_SPATIAL].filter(function (e) { return e === p.propertyValueType; }).length;
        };
        return KeyframeData;
    }());
    var c = {
    ...
    var l = {
        ...
        set_property: function (property, data) {
            ...
            var value = obj.value, expression = obj.expression, expressionEnabled = obj.expressionEnabled, keys = obj.keys;
            if (value != null) {
                if (property.name === 'Source Text') {
                    var val = value;
                    property.setValue(new TextDocument((_a = val.text) !== null && _a !== void 0 ? _a : ''));
            ...
            l.get_group(layer, data, l.recur_factory('get', {
                ...
                PropertyFn: function (p, d) {
                    switch (p.parentProperty.matchName) {
                        ...
                        case 'ADBE Transform Group': {
                            return (defaultFn(p, d) &&
                                ['X Position', 'Y Position'].filter(function (e) { return e === p.name; }).length === 0);
                        }
                        default:
                            return defaultFn(p, d);
                    }
    ...
    var p = {
        ...
        local: function () {
            return [Folder.appPackage.fsName, 'Scripts', 'ScriptUI Panels', config.name + '.jsx'].join('\\');
        },
    ...
    var a = {
        ...
        UI: function () {
            ...
            btn(win, File.decode(config.icons.setting)).onClick = function () {
                ...
                var win = u.dialog(null, config.name + ' ' + config.version).assign({
                    margins: 10
                });
        ...
        update: function () {
            ...
            system.callSystem('cmd /c powershell -Command "echo ' +
                config.name +
                '检查更新; iwr -Uri ' +
                config.updata_url +
                ' -OutFile ' +
            ...
            var version = (_a = f.read(file).match(/version: \'([\s\S]+?)\'/)) === null || _a === void 0 ? void 0 : _a[1];
